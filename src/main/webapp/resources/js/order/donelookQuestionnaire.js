 ...
 * 默认状态条件(有效)
 */
var default_statuses = "2";
/**
 * 默认条件(发生过完成带看)
 */
var doneLookCountStart = "1";
...
//               { field: 'phoneNum', title: '客户手机', width: 150, align: 'center', sortable: false },
               ...
               { field: 'lastLooKAt', title: '上次带看时间', width: 150, align: 'center', sortable: true,convert: convertDate },
...
$(function () {
    ...
    $('[name=lastLooKAt_begin]').datepicker({
        format : 'yyyy-mm-dd',
        todayHighlight:true,
        todayBtn:'linked'
    });

    $('[name=lastLooKAt_end]').datepicker({
        format : 'yyyy-mm-dd',
        todayHighlight:true,
        todayBtn:'linked'
    });
...
var clearSearchValue = function () {
    ...
    for (var i=0; i<conditions.length; i++){
        if(conditions[i].id == 'condition_customerCreateAt'){
            ...
            $('#created_at_end').val('');
        }else if(conditions[i].id == 'condition_lastLooKAt'){
            $('#lastLooKAt_begin').val('');
            $('#lastLooKAt_end').val('');
...
var getListGridParams = function(){
    ...
    var lastLooKAt_begin = $("#lastLooKAt_begin").val();
    if (lastLooKAt_begin != null && lastLooKAt_begin !=''){
        var begin = new Date(lastLooKAt_begin.replace(/-/g, ','));
        lastLooKAt_begin = parseInt(begin.getTime()/1000);
    }
    var lastLooKAt_end = $("#lastLooKAt_end").val();
    if (lastLooKAt_end != null && lastLooKAt_end !=''){
        var end = new Date(lastLooKAt_end.replace(/-/g, ','));
        lastLooKAt_end = parseInt((end.getTime() + 24*3600*1000)/1000);
    }
    ...
    var params = {
            ...
            'lastLooKAtBegin': lastLooKAt_begin,
            'lastLooKAtEnd': lastLooKAt_end,
...
var setSearchConditionsValue = function(){
    ...
    for (var i=0; i<conditions.length; i++){
        if (conditions[i].id == 'condition_customerCreateAt'){
            ...
            var callback_at_begin = $('#lastLooKAt_begin').val();
            var callback_at_end = $('#lastLooKAt_end').val();
            var value = '';
            if (callback_at_begin != null && callback_at_begin != '' && callback_at_end != null && callback_at_end != '') {
                value = '上次带看时间：' +callback_at_begin + '至' + callback_at_end;
            } else if (callback_at_begin != null && callback_at_begin != '' && (callback_at_end == null || callback_at_end == '')) {
                value = '上次带看时间：开始于' + callback_at_begin;
            } else if (callback_at_end != null && callback_at_end != '' && (callback_at_begin == null || callback_at_begin == '')) {
                value = '上次带看时间：截止于' + callback_at_end;
            }

            if (value != ''){
                $('#condition_lastLooKAt').css('display', 'inline');
                $('#condition_lastLooKAt').html(value);
            }else{
                $('#condition_lastLooKAt').css('display', 'none');
                $('#condition_lastLooKAt').html('');
            }
        }else if (conditions[i].id == 'condition_callBackAt'){
            var callback_at_begin = $('#callback_at_begin').val();
            var callback_at_end = $('#callback_at_end').val();
        ...
        }else if (conditions[i].id == 'condition_look_name'){
            var value = $('#look_name').val();
            if (value != null && value != ''){
                $('#condition_look_name').css('display', 'inline');
                $('#condition_look_name').html("带看顾问：" + value)
            }else{
                $('#condition_look_name').css('display', 'none');
                $('#condition_look_name').html('');
            }
...
var deSelectSearchConditons = function(obj){
    if(obj.id == 'condition_customerCreateAt'){
        ...
        $('#callback_at_begin').val('');
        $('#callback_at_end').val('');
    }else if(obj.id == 'condition_lastLooKAt'){
        $('#lastLooKAt_begin').val('');
        $('#lastLooKAt_end').val('');
