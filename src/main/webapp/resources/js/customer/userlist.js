 ...
 * 默认状态条件(有效)
 */
var default_statuses = "2,8,9";
var queryUrl = base+'/customer/queryNormalCustomer';
var addUrl = base+'';
var modifyUrl = base+'';
var deleteUrl = base+'';
 ...
 * 客服订单标识
 */
var service_order = "customer_customerAddByOrder";
var isShowAll = false;
var firstChoose = true;

var order = 'c.status desc, c.id';
...
//               { field: 'phoneNum', title: '客户手机', width: 150, align: 'center', sortable: true },
               ...
               { field: 'lastFollowTime', title: '最后跟进时间', width: 150, align: 'center', sortable: true,convert: convertDate },
...
function optRender(record){
	var ret = "<button type='button'  class='btn btn-default' onclick='initFollow(\""+ record.id +"\");'>跟进</button>&nbsp;";
    ...
    //recordStr += "}";
    ret += '<button type="button" value="带看"  class="btn btn-default" onclick="initLook('+ record.id + ')">带看</button>';
	return ret;
}
...
function nameRender(record, value){
    ...
    if(record.status == 8 || record.status == 9){
        tags += '<span class="msg-tag-close poshytooltip" title="无效客户正在由组长审核">审</span>&nbsp;';
    }

	if(record.systemresourceOriginal!=null&&record.systemresourceOriginal!=undefined&&record.systemresourceOriginal.indexOf(service_order)!=-1){
	...
	if(record.isPerfection=='1'){
		ret = "<a onclick='window.open(\"" +url +"\")' class='unline' style='cursor:pointer;'>" + " <font color='#28a4c9'>"+ value +"(未维护)</font></a><br/>" + tags ;
	}else if(!value){
		ret = "<a onclick='window.open(\"" +url +"\")' class='unline' style='cursor:pointer;'>" + " <font color='#28a4c9'>信息未完善</font></a><br/>" + tags ;
	}else{
		ret = "<a onclick='window.open(\"" +url +"\")' class='unline' style='cursor:pointer;'>" + " <font color='#28a4c9'>"+ value + "</font></a><br/>" + tags ;
	}
	return ret;
}
 ...
 * 打开为客户选择房源页面
 */
function initLook(id){
    var record = $('#' + grid_id)[grid_id].getCurSelectRecord(id);
	var search = base + '/customerlookController/fangyuanlist?customerId='+ record.id;
    if (record.customerName != null){
        search += "&customerName=" + encodeURI(record.customerName);
    }
    if (record.phoneNum != null){
        search += "&phoneNum=" + record.phoneNum;
    }
    if (record.orderPriceBegin != null){
        search += "&orderPriceBegin=" + record.orderPriceBegin;
    }
    if (record.orderPriceEnd != null){
        search += "&orderPriceEnd=" + record.orderPriceEnd;
    }
    if (record.orderAreaBegin!= null){
        search += "&orderAreaBegin=" + record.orderAreaBegin;
    }
    if (record.orderAreaEnd != null){
        search += "&orderAreaEnd=" + record.orderAreaEnd;
    }
    if (record.orderCityId != null){
        search += "&orderCityId=" + record.orderCityId;
    }
    if (record.orderDistrictId != null){
        search += "&orderDistrictId=" + record.orderDistrictId;
    }
    if (record.orderBusinesscircleId != null){
        search += "&orderBusinesscircleId=" + record.orderBusinesscircleId;
    }
    window.open(search);
}
...
//    initCityDistrictBusinessCircle(data, "city_combox_fangyuan", "district_combox_fangyuan", "businesscircle_combox_fangyuan");
    ...
    $('[name=lastFollowBegin]').datepicker({
    	format : 'yyyy-mm-dd',
    	todayHighlight:true,
    	todayBtn:'linked'
    });
    
    $('[name=lastFollowEnd]').datepicker({
    	format : 'yyyy-mm-dd',
    	todayHighlight:true,
    	todayBtn:'linked'
    });
...
var getListGridParams = function(){
	...
	var is_perfection = $("#is_perfection").val();
	var statuses = default_statuses;
	var isPerfection = null;
	if(is_perfection!=''||is_perfection!=null||is_perfection!=undefined){
		isPerfection = is_perfection;

        if (is_perfection != null && is_perfection != ''){
            if (is_perfection == '8,9'){
                statuses = '8,9';
                isPerfection = null;
            }else {
                statuses = '2';
    ...
    if (lastFollowBegin != null && lastFollowBegin !=''){
    	var begin = new Date(lastFollowBegin.replace(/-/g, ','));
    	lastFollowBegin = parseInt(begin.getTime()/1000);
    }
    var lastFollowEnd = $("#lastFollowEnd").val();
    if (lastFollowEnd != null && lastFollowEnd !=''){
    	var end = new Date(lastFollowEnd.replace(/-/g, ','));
    	lastFollowEnd = parseInt((end.getTime() + 24*3600*1000)/1000);
    }
    ...
    var params = {
            ...
            'lastFollowBegin':lastFollowBegin,
            'lastFollowEnd':lastFollowEnd,
...
var setSearchConditionsValue = function(){
    ...
    for (var i=0; i<conditions.length; i++){
        ...
        }else if(conditions[i].id =='condition_is_perfection'){
        	var value = $("#is_perfection").val();
        	var value_text = $("#is_perfection option:selected").text();
        	if (value != null && value != ''){
                $('#condition_is_perfection').css('display', 'inline');
                $('#condition_is_perfection').html("客户筛选：" + value_text)
            }else{
                $('#condition_is_perfection').css('display', 'none');
                $('#condition_is_perfection').html('');
            }
...
var deSelectSearchConditons = function(obj){
    if (obj.id == 'condition_creatTime'){
    	...
    	$("#id").val('');
    }else if(obj.id == 'condition_is_perfection'){
    	$("#is_perfection").val('');
